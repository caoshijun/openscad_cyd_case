...
if (isdrawbase) { 
    difference(){
        union(){
            ...
            up(wallz) cuboid([L,W,gapz],rounding=R,edges="Z",anchor=BOT) position(TOP) //gap
            grid_copies([support_pin_length,support_pin_wildth], n=[2,2]) cyl(h=H1-gapz-wallz,d=support_pin_d1,chamfer1=-1.6,anchor=BOT) position(TOP) cyl(h=pcb_thickness+intel_height_size,d=support_pin_d2,chamfer1=-0.4,chamfer2=0.4,anchor=BOT);  //支撑柱及定位孔
    ...
    up(wallz+gapz) difference(){
        rect_tube(h=H-wallz-gapz, size=[L,W], isize=[L2,W2],rounding=R, irounding=R,$fn=32);    //圆角矩形管状物
        up(H-wallz-gapz-intel_height_size) rect_tube(h=intel_height_size, size=[L,W],isize=[L1,W1],rounding=R,irounding=R,$fn=32,anchor=BOT); //圆角矩形圈切出交叉区
        up(H-wallz-gapz-intel_height_size) rect_tube(h=intel_height_size, size1=[L,W],size2=[L1+1,W1+1],rounding=R,irounding=R,wall=1,$fn=32,anchor=BOT); //交叉区铲出斜坡
        up(H-wallz-gapz-intel_height_size+snip_depth/2) teardrop_round_rect(L1+snip_depth/2,W1+snip_depth/2,R,snip_depth); //在交叉带上挖槽用于上盒嵌入
